...
action :enable do
  t = template ['nginx']['config']['sites_available']+new_resource.name do
    ...
    variables({
      ...
      :serveroptions => node['nginx']['server']['default'] + node['nginx']['server'][new_resource.name] + new_resource.serveroptions
    })
  end
